...
def test_cmd_repair_error_reading_points_to_from_sqlite_not_remine(
    ...
    # Local palace-isolation: cmd_repair resolves via _resolve_cli_palace ->
    # resolved_palace_path(); configure it so the mocked config points at the
    # test palace rather than returning a bare MagicMock.
    mock_config_cls.return_value.resolved_palace_path.return_value = str(palace_dir)
...
def _repair_backend_mocks(mock_config_cls, palace_dir, create_collection_results=None):
    ...
    """
    mock_config_cls.return_value.palace_path = str(palace_dir)
    # Merged cmd_repair resolves the palace via ``resolved_palace_path()``
    # (palace-isolation walk-up), not the bare ``palace_path`` attribute, so
    # the mock must answer that call too or resolution returns an unconfigured
    # MagicMock and cmd_repair reports "No palace found".
    mock_config_cls.return_value.resolved_palace_path.return_value = str(palace_dir)
    mock_config_cls.return_value.collection_name = "mempalace_drawers"
    mock_col = MagicMock()
    mock_col.count.return_value = 2
...
def test_cmd_sync_palace_dir_no_db(mock_config_cls, tmp_path, capsys):
    ...
    # Local cmd_sync resolves the palace via _resolve_cli_palace ->
    # resolved_palace_path(); wire that (not just palace_path) so the State
    # B branch sees a real dir without chroma.sqlite3.
    _mock_config(mock_config_cls, str(tmp_path))
    args = argparse.Namespace(palace=None, dir=None, root=[], wing=None, dry_run=False)
    cmd_sync(args)
    captured = capsys.readouterr()
...
def test_cmd_repair_rebuild_index_alias_uses_sqlite_archive(mock_config_cls, tmp_path):
    ...
    palace_dir.mkdir()
    mock_config_cls.return_value.palace_path = str(palace_dir)
    # Local palace-isolation: with args.palace set, cmd_repair resolves via
    # _resolve_cli_palace -> resolve_palace(alias-or-path); configure it so
    # the mocked config points at the test palace rather than returning a
    # bare MagicMock.
    mock_config_cls.return_value.resolve_palace.return_value = str(palace_dir)
