...
def test_scan_project_respects_nested_mempalaceignore():
    tmpdir = tempfile.mkdtemp()
    try:
        project_root = Path(tmpdir).resolve()

        write_file(project_root / ".mempalaceignore", "*.log\n")
        write_file(project_root / "subrepo" / ".mempalaceignore", "tasks/\n")
...
def test_scan_project_allows_nested_mempalaceignore_override():
    tmpdir = tempfile.mkdtemp()
    try:
        project_root = Path(tmpdir).resolve()

        write_file(project_root / ".mempalaceignore", "*.csv\n")
        write_file(project_root / "subrepo" / ".mempalaceignore", "!keep.csv\n")
...
def test_scan_project_allows_mempalaceignore_negation_when_parent_dir_is_visible():
    tmpdir = tempfile.mkdtemp()
    try:
        project_root = Path(tmpdir).resolve()

        write_file(project_root / ".mempalaceignore", "generated/*\n!generated/keep.py\n")
...
def test_scan_project_can_disable_mempalaceignore():
    ...
    try:
        ...
        assert scanned_files(project_root, respect_ignore=False) == ["data/stuff.csv"]
    finally:
        shutil.rmtree(tmpdir)
...
def test_scan_project_can_include_ignored_directory():
    ...
    try:
        ...
        write_file(project_root / ".mempalaceignore", "docs/\n")
...
def test_scan_project_can_include_exact_file_without_known_extension():
    ...
    try:
        ...
        write_file(project_root / ".mempalaceignore", "README\n")
...
def test_scan_project_skip_dirs_still_apply_without_override():
    ...
    try:
        ...
        assert scanned_files(project_root, respect_ignore=False) == ["main.py"]
    finally:
        shutil.rmtree(tmpdir)
...
def test_scan_project_logs_skipped_symlinks(tmp_path, capsys):
    ...
    files = scanned_files(project_root, respect_ignore=False)
...
def test_process_file_uses_bounded_upsert_batches(tmp_path, monkeypatch):
    ...
    class FakeCol:
        ...
        def upsert(self, documents, ids, metadatas, embeddings=None):
            ...
            self.embedding_batch_sizes.append(len(embeddings) if embeddings is not None else None)
...
@pytest.mark.skip(
    ...
    "mine bypasses for non-dry runs (it routes through _prepare_file / _embed_prepared / "
    "_write_prepared — see divergence.md the parallel-mine divergence). The identical summary-formatting assertions "
    "are exercised through the retained serial dry-run path by "
    "test_mine_dry_run_summary_counts_chunk_cap_drops; the _mine_impl summary block that "
    "emits these lines is shared across both modes."
)
...
def test_mine_plumbs_max_chunks_per_file_to_process_file(tmp_path):
    ...
    ``cmd_mine -> mine -> _mine_impl -> _prepare_file``.

    The local parallel-pipeline mine (non-dry) routes through
    ``_prepare_file`` (the producer entry), not ``process_file`` — so we
    patch the producer entry and assert the already-resolved cap (``0``
    from ``_resolve_max_chunks_per_file(0)``) reaches it. Returning ``None``
    makes it a silent skip, so no embed/write is needed."""
    from unittest.mock import patch
    ...
    def fake_prepare_file(*args, **kwargs):
        captured["max_chunks_per_file"] = kwargs.get("max_chunks_per_file")
        return None

    with patch("mempalace.miner._prepare_file", side_effect=fake_prepare_file):
