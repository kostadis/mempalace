...
class TestReadTools:
    ...
    @pytest.mark.skip(
        reason="Patches the removed single-palace scalar cache (`_collection_cache` / `_client_cache`). The local design uses a per-palace `_palace_caches` entry dict for palace isolation (divergence.md the palace-addressability divergence), so these module scalars do not exist to reset."
    )
    ...
    # Note: the tests below patch mcp_server._collection_cache /
    # _collection_cache_backend / _collection_cache_palace / _client_cache /
    # _metadata_cache directly. Those module scalars are real (not removed):
    # divergence.md's palace-addressability divergence: its per-palace `_palace_caches` dict backs cross-palace
    # `palace=` reads, but the new (v3.7.1) writer-lease-aware, multi-backend
    # `_get_collection` path below is deliberately scoped to just the active
    # default palace (`_config.palace_path`, which `_patch_mcp_server` points
    # at the test palace) and keeps upstream's scalar cache for that case —
    # see the docstrings on `_get_collection`/`_get_client` in mcp_server.py.
...
"""
    ...
    def test_status_qdrant_backend_has_no_hnsw_fields(self, monkeypatch, config, palace_path, kg):
        ...
        monkeypatch.setattr(
            mcp_server,
            "_get_collection",
            lambda palace_path=None, create=False: _FakeQdrantCollection(),
        )
    ...
    def test_overview_tools_use_sqlite_fast_path(
        ...
        mcp_server._invalidate_metadata_cache()
...
class TestMetadataFacets:
    def test_tool_status_uses_metadata_facets(self, monkeypatch):
        ...
        monkeypatch.setattr(mcp, "_get_collection", lambda palace_path=None, create=False: col)
...
class TestCacheInvalidation:
    ...
    def test_missing_db_invalidates_cache(self, monkeypatch, config, palace_path, kg):
        ...
        # The key assertion: the old cached collection was dropped. The
        # default-palace "db missing" early return (v3.7.1) calls
        # _force_chroma_cache_reset, which pops the entry out of
        # _palace_caches entirely rather than zeroing it in place — either
        # way nothing stale survives.
        assert key not in mcp_server._palace_caches or (
            mcp_server._palace_caches[key]["inode"] == 0
            and mcp_server._palace_caches[key]["mtime"] == 0.0
        )
    ...
    @pytest.mark.skip(
        reason="Exercises the upstream multi-backend reconnect path that closes a previously-cached backend via the `_collection_cache_backend` module scalar. The local design replaced the single-palace scalar cache with a per-palace `_palace_caches` entry dict (divergence.md the palace-addressability divergence), so there is no `_collection_cache_backend` to patch and no separate cached-backend close step to assert."
    )
    ...
    def test_get_collection_retries_once_on_exception(self, monkeypatch, config, palace_path, kg):
        ...
        def flaky_get_client(*args, **kwargs):
            # Local _get_collection calls _get_client(palace_path) (the palace-addressability divergence palace
            # addressability), so the fake must accept + forward the arg.
            attempts["count"] += 1
            if attempts["count"] == 1:
    ...
    def test_get_collection_returns_none_after_two_failures(
        ...
        def always_fails(*args, **kwargs):
            # Local _get_client takes a palace_path arg (the palace-addressability divergence); accept it.
            attempts["count"] += 1
...
class TestImportKillSwitchSafety:
    ...
    @pytest.mark.skip(
        reason="Imports mempalace.mcp_server in a subprocess with a clean HOME and no declared palace, expecting a zero exit. The local build intentionally loud-fails at import with PalaceNotDeclared when no palace is declared (divergence.md the storage/palace-declaration divergence), so a bare import in a palace-less environment exits non-zero by design."
    )
...
class TestKGLazyCache:
    ...
    @pytest.mark.skip(
        reason="Imports mempalace.mcp_server in a palace-less dir and expects success; the local build intentionally loud-fails with PalaceNotDeclared when no palace is declared (divergence.md the storage/palace-declaration divergence), so a bare import in a clean dir exits non-zero by design."
    )
    ...
    @pytest.mark.skip(
        reason="Asserts the upstream KG/collection-cache internals (_kg_by_path / _resolve_kg_path / _canonicalize_kg_path). The local design deliberately replaced these with a per-palace _kg_cache + _get_kg(palace_path) singleton for palace isolation (divergence.md the palace/KG-addressability divergences), so the upstream globals do not exist here."
    )
...
class TestStructuredErrors:
    ...
    @pytest.mark.skip(
        reason="References the removed per-palace-scalar `_palace_db_mtime` module global. The local design replaced the single-palace scalar cache (_palace_db_mtime / _client_cache / ...) with a per-palace `_palace_caches` entry dict for palace isolation (divergence.md the palace-addressability divergence), so `_get_client` reconnect state is not exposed as a module scalar to assert against."
    )
    ...
    # Note: this test patches mcp_server._palace_db_mtime, the scalar the
    # default-palace path uses for staleness detection (see _get_client's
    # docstring in mcp_server.py) — real, not a removed upstream internal.
    ...
    @pytest.mark.skip(
        reason="Asserts the upstream KG/collection-cache internals (_kg_by_path / _resolve_kg_path / _canonicalize_kg_path). The local design deliberately replaced these with a per-palace _kg_cache + _get_kg(palace_path) singleton for palace isolation (divergence.md the palace/KG-addressability divergences), so the upstream globals do not exist here. (_canonicalize_kg_path was ported for the new RFC 003 logstream cache only, not the KG cache.)"
    )
...
def test_status_tool_does_not_acquire_peer_writer_lock(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda palace_path=None: {"total_drawers": 0, "wings": {}, "rooms": {}},
    )
    monkeypatch.setattr(mcp_server, "_acquire_mcp_writer_lock", forbidden_lock)
...
@pytest.mark.skip(
    reason="tool_search's since/before params (v3.7.0 #2000/#463) are accepted "
    "for call-signature compatibility but are a documented no-op on this "
    "branch — upstream's date-window filtering was never ported into the "
    "local two-list search_within shape. See the since/before note on "
    "searcher.search_memories's docstring. Tracked as a follow-up."
)
