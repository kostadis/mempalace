...
@pytest.mark.skip(
    reason='Exercises `candidate_strategy="union"` and its `unsupported_capability` reporting. The local search design deliberately dropped union in favor of the two-list primary/themes split (divergence.md\'s closet-boosting divergence), so `search_memories` does not even accept `candidate_strategy`, and the union path/capability fields are never produced.'
)
