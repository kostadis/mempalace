...
@pytest.mark.skip(
    reason='Exercises `candidate_strategy="union"`, the hybrid BM25/lexical candidate-union path. The local search design deliberately dropped union in favor of the two-list primary/themes split (divergence.md the closet-boosting divergence), so `candidate_strategy` is a no-op and union-only fields (`matched_via="bm25_backend"`) are never produced.'
)
...
@pytest.mark.skip(
    reason='Exercises `candidate_strategy="union"` and its `unsupported_capability` reporting. The local search design deliberately dropped union in favor of the two-list primary/themes split (divergence.md the closet-boosting divergence), so the union path and its capability fields are never produced.'
)
...
@pytest.mark.skip(
    reason='Asserts multi-backend vector-disabled capability reporting (`unsupported_capability="chroma_hnsw_fallback"` + `backend`) that arrived with the union/backend-aware search work. The local two-list search keeps a chroma-sqlite BM25 fallback and does not emit multi-backend capability fields (divergence.md the closet-boosting/search divergence).'
)
