...
@pytest.mark.skip(
    reason="Calls searcher._finalize_candidate_hits directly. The local search "
    "design deliberately dropped the union candidate-merge machinery in favor "
    "of the two-list primary/themes split (divergence.md's closet-boosting divergence) — "
    "_finalize_candidate_hits doesn't exist in this fork's searcher.py anymore."
)
...
@pytest.mark.skip(
    reason="search_memories's since/before params (v3.7.0 #2000/#463) are "
    "accepted for call-signature compatibility but are a documented no-op on "
    "this branch — upstream's date-window filtering (_window_and_fallback_gate, "
    "candidate-pool widening, date_filter_pool_truncated reporting) was never "
    "ported into the local two-list search_within shape. See the since/before "
    "note on searcher.search_memories's docstring. Tracked as a follow-up."
)
