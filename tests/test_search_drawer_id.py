...
@pytest.mark.skip(
    reason='Exercises _finalize_candidate_hits and candidate_strategy="union" '
    "directly. The local search design deliberately dropped the union "
    "candidate-merge machinery in favor of the two-list primary/themes split "
    "(divergence.md's closet-boosting divergence) — _finalize_candidate_hits doesn't exist in this "
    "fork's searcher.py at all anymore."
)
def test_union_results_use_parent_or_lexical_hit_id():
    ...
    hits, error = _finalize_candidate_hits(  # noqa: F821 — skipped, see decorator
