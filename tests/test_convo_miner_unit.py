...
class TestWritePreparedConvo:
    ...
    def test_uses_bounded_upsert_batches(self, monkeypatch):
        ...
        class FakeCol:
            ...
            def get(self, *args, **kwargs):
                # The write phase now runs an extract-mode-scoped delete that
                # queries existing drawer IDs first; return an empty page.
        ...
        embeddings_batches = [[[0.0, 0.0, 0.0] for _ in batch.documents] for batch in batches]
    ...
    def test_prepare_convo_populates_entities_metadata(self, tmp_path, monkeypatch):
        """entities_metadata() output lands in every chunk's metadata.

        Post-refactor equivalent of the old ``_file_chunks_locked`` version of
        this test: entity extraction now happens in ``_prepare_convo`` (which
        owns chunk-metadata construction), not the write phase.
        """
        import mempalace.convo_miner as convo_miner
        ...
        f.write_text(
            ...
            + ("Extra context to clear the minimum chunk size. " * 3)
            + "\n",
            encoding="utf-8",
        )

        prepared = convo_miner._prepare_convo(
            f, "wing", "agent", "exchange", False, collection=None
        )
        ...
        for batch in prepared.batches:
            for meta in batch.metadatas:
                entities.update(meta["entities"].split(";"))
...
class TestMineConvosWorkersPlumbing:
    ...
    @pytest.mark.skip(
        reason="Tests upstream's monolithic _file_chunks_locked directly. Local "
        "convo mining is split into _prepare_convo/_write_prepared_convo "
        "(divergence.md's mining-pipeline divergence); _file_chunks_locked doesn't exist here. The #105 "
        "abort-on-purge-failure protection this pins should be re-verified "
        "against _write_prepared_convo rather than reviving this function."
    )
    def test_aborts_when_stale_drawer_purge_fails(self, monkeypatch):
        ...
        drawers, room_counts, skipped = _file_chunks_locked(  # noqa: F821 — skipped, see decorator
    ...
    @pytest.mark.skip(
        reason="Tests upstream's monolithic _file_chunks_locked directly. Local "
        "convo mining is split into _prepare_convo/_write_prepared_convo "
        "(divergence.md's mining-pipeline divergence); _file_chunks_locked doesn't exist here. The #2183 "
        "chunk_total protection this pins should be re-verified against "
        "_write_prepared_convo rather than reviving this function."
    )
    def test_stamps_chunk_total_for_completion_check(self, monkeypatch):
        ...
        _file_chunks_locked(  # noqa: F821 — skipped, see decorator
            col, "chat.txt", chunks, "wing", "general", "agent", "exchange"
        )
    ...
    @pytest.mark.skip(
        ...
        "convo mining is split into _prepare_convo/_write_prepared_convo "
        "(divergence.md's mining-pipeline divergence); _file_chunks_locked doesn't exist here. The #2183 "
        "partial-drawer cleanup this pins should be re-verified against "
        "_write_prepared_convo rather than reviving this function."
    )
