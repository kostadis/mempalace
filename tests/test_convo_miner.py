...
@pytest.mark.skip(
    reason="Asserts the serial convo mine's count-only-new-work limit semantics. "
    "The parallel convo pipeline applies --limit as a pre-slice of the scanned "
    "file list rather than stopping after N files that produced new drawers, so "
    "the already-mined-skip accounting under a limit differs from the serial "
    "loop this test pins."
)
...
@pytest.mark.skip(
    reason="Content-hash dedup (v3.7.0 #2050) is now wired for the whole-file "
    "case (test_mine_convos_skips_same_content_under_new_filename passes), but "
    "this test needs PER-CONVERSATION hashing within one multi-conversation "
    "bundle file — palace.prefetch_content_hashes already supports the "
    "comma-joined multi-hash-per-drawer-set shape for this, but "
    "_prepare_convo does not yet split a bundle into per-conversation pieces "
    "before hashing. Tracked as a follow-up."
)
