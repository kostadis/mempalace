            ...
            if mcp_server is not None:
                ...
                if hasattr(mcp_server, "_palace_caches"):
                    ...
                    if callable(close_entry):
                        for entry in mcp_server._palace_caches.values():
                            close_entry(entry)
                    mcp_server._palace_caches.clear()
                # The v3.7.1 default-palace multi-backend/writer-lease path
                # (mcp_server.py: _get_client/_get_collection) keeps its own
                # scalar cache alongside _palace_caches (the palace-addressability divergence).
                # Real code execution — not just monkeypatch — mutates these,
                # so leaving them set would leak state (e.g. a stale
                # _collection_open_error) into unrelated later tests.
                ...
                for scalar, default in (
                    ...
                    if hasattr(mcp_server, scalar):
                        setattr(mcp_server, scalar, default)
