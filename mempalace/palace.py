...
#
# This MUST be process-wide, not thread-local: fcntl.flock is per-process, so
# re-entrant credit must be process-wide too. Worker threads inside a parallel
# mine pipeline need to pass through ``ChromaCollection._write_lock`` without
# re-acquiring the outer lock the orchestrator thread is holding, and the MCP
# HTTP transport (ThreadingHTTPServer) acquires the long-lived writer-lease on
...
# by ``_palace_lock_guard`` because the set is shared across threads.
#
