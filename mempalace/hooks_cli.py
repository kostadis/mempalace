...
def _wing_from_transcript_path(transcript_path: str) -> str:
    ...
    For Claude Code we extract ``<project>`` after ``-Projects-``. For
    Cursor we take the trailing path segment of the encoded project path
    (the actual project directory name) so ``Users-jp-src-myapp`` →
    ``wing_myapp``. Returns ``wing_<project>`` to match the AAAK_SPEC
    convention; falls back to ``wing_sessions``.
    """
    # Normalize path separators for cross-platform (Windows backslashes)
    normalized = transcript_path.replace("\\", "/")
    # Claude Code: ~/.claude/projects/-home-jp-Projects-<project>/...
