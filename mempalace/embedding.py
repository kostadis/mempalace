...
* ``openai-compat`` — embeddings served by any OpenAI-compatible
  ...
  for the cosine collection; the dimension is whatever the server returns.
  Stays local when the endpoint is on your machine/LAN.

Switching providers (or models within a provider) requires wiping and
re-mining the palace (``mempalace repair rebuild-index``): vectors at one
dimension cannot be queried by an EF that produces a different dimension.
...
def describe_device(device: Optional[str] = None) -> str:
    ...
    which provider/device is doing the work — or, for a remote provider,
    that embeddings are served by a remote endpoint rather than local
    hardware (in which case the ``embedding_device`` accelerator label is
    irrelevant).
    ...
    # The upstream (v3.7.0) model-based openai-compat selector — reached
    # when embedding_provider is onnx/unset but embedding_model itself is
    # "openai-compat" (embedding_api_url/embedding_api_model/embedding_api_key
    # in config). See the matching branch in get_embedding_function().
    if str(getattr(config, "embedding_model", "") or "").strip().lower() == "openai-compat":
        url = getattr(config, "embedding_api_url", None)
