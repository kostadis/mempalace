...
def _discard_mcp_storage_handles() -> None:
    ...
    # Also drop the per-palace dict entry (palace-addressability divergence) for the default palace: chroma
    # opens are cached there (mirrored into the scalar ``_client_cache`` only
    # when the resolved palace is the default one — see ``_get_client``), so
    # without this a stale dict-cached chroma client would survive the
    # ownership transition even though the scalar below gets cleared.
...
def _canonicalize_kg_path(path: str) -> str:
    """Canonicalize a cache key so path aliases collapse onto one entry.

    ``realpath`` resolves symlinks: two tenants pointing at the same
    SQLite file via different layouts (``/srv/A`` and ``/srv/link-to-A``)
    hit a single cached handle rather than opening duplicate connections.
    ``normcase`` normalizes Windows drive-letter casing (``C:\\palace`` vs
    ``c:\\palace``) and path-separator style; on POSIX it returns the input
    unchanged. Ported from upstream for the new RFC 003 logstream cache
    (:func:`_get_logstream`); local's ``_kg_cache``/``_get_kg`` (the
    KG-cache divergence) still key on plain ``abspath``+``expanduser`` —
    see divergence.md deferred item #2.
    """
    return os.path.normcase(os.path.realpath(path))


# Scalar cache used only by the writer-lease-aware, multi-backend-aware
# default-palace path added in v3.7.1 (``_get_client``/``_get_collection``
# below, plus ``_discard_mcp_storage_handles``). Deliberately NOT folded into
# ``_palace_caches``: the writer lease this backs is inherently scoped to the
# one palace this MCP process was launched against (``_config.palace_path``)
# — a ``palace=`` cross-palace read for some other palace can never be "this
# server's writer" for that palace, so cross-palace reads keep using the
# plain per-palace dict-cache path above instead. See divergence.md's
# palace-addressability divergence notes.
_client_cache = None
_collection_cache = None
_collection_cache_backend = None
_collection_cache_palace = None
# Populated by the default-palace multi-backend path below on backend
# mismatch / open failure, and consumed by ``_collection_error_or_no_palace``.
# Cross-palace reads via a non-default ``palace=`` arg still go through the
# older chroma-only per-palace-cache path, which never sets this — those
# calls degrade to the plain ``_no_palace()`` message instead, same as
# before v3.7.1.
_collection_open_error = None
# inode/mtime of chroma.sqlite3 for the default palace at scalar-cache time —
# the staleness signal ``_get_client`` compares against on every call for the
# default palace (mirrors ``entry["inode"]``/``entry["mtime"]`` in
# ``_palace_caches``, which back the same check for a non-default
# cross-palace ``palace=`` read).
_palace_db_inode = 0
_palace_db_mtime = 0.0


def _close_cached_client(entry: dict) -> None:
    """Close (not just dereference) a cache entry's chromadb client.

    A bare deref leaves the rust-side file handles open; on Windows those
    stay locked and leak across the session (#1128). Callers still
    overwrite/clear ``entry["client"]``/``entry["collection"]`` themselves
    right after calling this.
    """
    cached_client = entry.get("client")
    if cached_client is not None:
        close = getattr(cached_client, "close", None)
        if callable(close):
            try:
...
def _force_chroma_cache_reset() -> None:
    ...
    # state. Without clearing _DEFAULT_BACKEND._clients the retry would
    # just hit the same stale handle, since tool_search routes via
    # search_memories -> palace.get_collection -> backend cache.
    global \
        _client_cache, \
        _collection_cache, \
        _collection_cache_backend, \
...
def tool_status(palace: str = None):
    ...
    is_default = resolved == _default_palace_path()

    # SQLite integrity gate: if the active default palace's chroma.sqlite3
    # fails PRAGMA quick_check, avoid touching chromadb (a corrupt FTS/HNSW
    # segment can segfault the client) and answer from sqlite with an error
    # flag. Only the active default palace is guarded here — a cross-palace
    # ``palace=`` status uses the client path directly.
    if is_default:
        _ensure_sqlite_integrity_status()
        if _sqlite_integrity_errors:
    ...
    try:
        db_exists = detect_backend_for_path(resolved) is not None
    except Exception:
        logger.debug("backend artifact detection failed", exc_info=True)
        db_exists = False
    if is_default:
    ...
    # client path below. Gated to the active default palace — ``_sqlite_taxonomy``
    # reads ``_config.palace_path``, so a cross-palace ``palace=`` status must
    # use the client path against ``resolved`` instead.
...
def tool_reconnect():
    ...
    if _is_chroma_backend():
        try:
            ...
            if callable(clear_system_cache):
                ...
                logger.debug(
                    "SharedSystemClient.clear_system_cache is unavailable; "
                    "skipping shared Chroma cache clear during reconnect"
                )
        except Exception as exc:
            logger.debug(
    ...
    # Re-arm the per-process HNSW quarantine gate so the safety probe re-runs
    # on the next open after an out-of-band write (#1573). The set is a
    # ChromaBackend class attribute reachable without importing chromadb, so
    # this is safe on turbovec-only deploys.
